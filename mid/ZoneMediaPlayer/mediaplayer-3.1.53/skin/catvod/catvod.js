...
(function(){
    ...
    var IS_DEBUG = 'true'==='false' ? true : false;
    var UPDATE_NUM = 1;
    var D = window.getDframework();
    var Media = D.module('imgtech.media.Media');
	
	window.createNewVersionMediaPlayer = function(qid, opt){
		
		var attr = opt.addattr;
		var source = {
			src: opt.sources[0].src,
			autoplay: true,
			// continueTime: 0, // 이어보기를 묻고 이어보기 동작(value 값 숫자, 문자열시 비정상)
			// autoContinueTime: 0,  // 자동으로 이어보기 동작(value 값 숫자, 문자열시 비정상)
		};
		
		if(attr) {
			if(attr.currentTime) {
				source.autoplay = false;
				source.autoContinueTime = attr.currentTime;
			}
			...
			if(attr.qualityYN) {
				source.quality = attr.qualityArrSrc;
			}
		}
		
		window.createMediaPlayer(qid, source, {
			addClass: 'newVersion-preview',
			module: {
				layout: {
				...
				rate: {
					use: true,
					rateBtn: {hideButton: true, hideExtendButton: true, hideMenuButton: true},
					rateDownBtn: {hideButton: true, hideExtendButton: true, hideMenuButton: true},
					rateUpBtn: {hideButton: true, hideExtendButton: true, hideMenuButton: true},
					rateText: {hideButton: true, hideExtendButton: true, hideMenuButton: true},
				},
				bookmark: {
					addBtn: {hideButton: true}
				},
				config: {
					configBtn: {hideButton: true}
				},
				repeat: {
			...
			*/
			...
			switch(e.type){
				case this.EVENT.ON_READY:
					var volUI = D.query(this.userElement, '.mpv-volume-layout');
					var fsUI = D.query(this.userElement, '.mpv-fullscreen-btn');
					D.after(fsUI, volUI);
					
					if(attr) {
						if(attr.thumbImg) {
							$(".mpv-player").append(attr.thumbImg);
							$(".mpv-player > img").css("position", "initial");
						}
					}
					break;
					
				case this.EVENT.FULLSCREEN_CHANGE:
					if(attr) {
						if(attr.clipCourseYn){
							if(e.data){
								// enter fullscreen
								$(".lect_top").removeClass("mob_view");
							}else{
								// leave fullscreen
								$(".lect_top").addClass("mob_view");
							}
						}
					}
					break;
				...
				case this.EVENT.STATE_CHANGE:
					if(e.data==this.STATE.ENDED){
						if(attr) {
							if(attr.onMediaEnded){
								onMediaEnded();
							} else if(attr.onMediaPrerollEnded){
								onMediaPrerollEnded();
							} else {
								this.attr.src.autoplay = false;
							}
						}
					}
					break;
    ...
    window.createFiveMinVersionMediaPlayer = function(qid, opt){
        ...
        var source = {
            ...
            // continueTime: 0, // 이어보기를 묻고 이어보기 동작(value 값 숫자, 문자열시 비정상)
            // autoContinueTime: 0,  // 자동으로 이어보기 동작(value 값 숫자, 문자열시 비정상)
            disableSeekToFirst : true,
            virtualTime:{start: 0, end: 300}
        };

        if(attr){
            if(attr.previewYn == 'Y'){
                source.virtualTime = {start: 0, end: 300};// 5분만 미리보기 설정
            }
        }
            ...
            module: {
                continueTime: {
                    ...
                    message: function(data){ 
                    	var str = '최종학습 위치에서 이어보기를 하시겠습니까?\n\n' + 
                    				source.courseName + '\n\n' + 
                    				source.lectName + ' (' + Media.secondsToTimeString(data) + ')';
                    	return str;  
                    }
                },
                layout: {
                ...
                repmark: { // 구간반복 북마크 모듈
                    ...
                    , add: function(data){
						...
						$.ajax( {
							url : "/pleasure/course/player/saveMarkAjax?" + p[1],
							data : {
							...
							success : function( m ) {
								if ( m.state == "fail" ) {
									...
									return false;
								} else if ( m.state == "param" ) {
									alert( "필수 입력값이 부족합니다." );
									return false;
								} else if ( m.state == "success" ) {
									if ( location.hash === "#webmark" ) {
										$( window ).hashchange();
									} else {
										window.location.hash = "webmark";
									}
								}
							},
                ...
                playlist: {
                    useConfigList: false // 플레이리스트를 설정 목록에 표시 유무 입니다.
    ...
    function getHtmlResource(param){
        ...
        h.push("          <div class='mpv-repmark-menu-btn mpv-button mpv-bctrl-btn' title='웹갈피 북마크 목록 버튼'></div>\n");
        h.push("          <div class='mpv-repmark-add-btn mpv-button mpv-bctrl-btn' title='웹갈피 북마크 추가 버튼'></div>\n");
        ...
        h.push("                <div class='mpv-repmark-menu-title-nm'>웹갈피 북마크</div>");
        h.push("                <div class='mpv-view-close-btn mpv-button mpv-menu-close-btn' title='닫기'></div>");
        h.push("                <div class='mpv-repmark-add-btn mpv-button mpv-in-box' title='웹갈피 북마크 추가 버튼'></div>");
        h.push("              </div>");
        h.push("              <div class='mpv-view-rows'>");
        h.push("                <div class='mpv-view-item'>");
        ...
        h.push("          <div class='mpv-repmark-subject-nm'>웹갈피 북마크 입력</div>\n");
        h.push("        </div>\n");
...
/* 크롬 업데이트 적용 2023-05-08 */
(function(){

    var D = getDframework();

    var WatchTimeMod = D.module('imgtech.media.module.WatchTimeEx');

    WatchTimeMod.prototype._initBeforeUnload = function(){

        var thiz = this;

        if(D.b.firefox){

            return D.on(window, 'unload', function(e){
            ...
            D.on(window, 'beforeunload', function(e){
