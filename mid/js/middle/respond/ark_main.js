...
var arkPath = "http://ark.ebs.co.kr/ark";						// 자동완성 경로
...
var IE7_LEFT_OFFSET = -1;				// IE7 일 경우 LEFT 옵셋 값 오차 조정
...
var arkTop = -1;								// 자동완성 상단에서의 위치 값을 설정한다.
var arkLeft = 0;								// 자동완성 왼쪽에서의 위치 값을 설정한다.
var arkImgTop = 5;								// 자동완성 화살표 이미지의 상단에서 위치 값을 설정한다.
var arkImgLeft = 27;							// 자동완성 화살표 이미지의 왼쪽에서 위치 값을 설정한다.
var tooltip01TopPos = -10;						// 자동완성 기능끄기 툴팁의 상단 기준 위치 오차 조정값
var tooltip01LeftPos = -10;					// 자동완성 기능끄기 툴팁의 좌측 기준 위치 오차 조정값
		...
		if ($.browser.opera || $.browser.mozilla) {
			...
			$(document).keyup(function(event) {
				...
				if (event.keyCode == 38 || event.keyCode == 40) {
					...
					if ($(event.target).is(queryId)) {
						if (isArk && trim($(queryId).val()) != "") {
							if(datatype == "json") {
								requestArkJson($(queryId).val());
							} else if(datatype == "xml") {
								requestArkXml($(queryId).val());
							}
						} else if (trim($(queryId).val()) == "") {
							hideArk();
						}
					}
				}
...
function drawArk() {
	...
	// 자동완성 접기/펼침 이미지 생성
	str = "<div id=\"" + arkDownId + "\" style=\"position:absolute; display:block; cursor:pointer;\"><button><img id=\"" + imgDownId + "\" src=\"http://static.ebs.co.kr/www/common/img/global/icon/arrow_auto.gif\" alt=\"자동완성펼치기\"></button></div>";
	str += "<div id=\"" + arkUpId + "\" style=\"position:absolute; display:none; cursor:pointer;\"><button><img id=\"" + imgUpId + "\" src=\"http://static.ebs.co.kr/www/common/img/global/icon/arrow_auto2.gif\"  alt=\"자동완성접기\" ></button></div>";
	
	// 툴팁 이미지 생성
	str += "<div id=\"tooltip01\" style=\"position:absolute; display:none; cursor:pointer; z-index:999999;\"><img id=\"tooltipIcon01\" src=\"http://static.ebs.co.kr/www/common/img/global/icon/tooltip_01.gif\" alt=\"자동완성접기\" /></div>";
	...
	try
	{
		...
		if (isArk) {
			...
			if ($(queryId).val() != "" && isListShow) {
				...
				$("body").append("<span style=\"display:none;\"><div id=\"tooltip02\" style=\"position:absolute;\"><img id=\"tooltipIcon02\" src=\"http://static.ebs.co.kr/www/common/img/global/icon/tooltip_02.gif\" /></div></span>");
			} else {
